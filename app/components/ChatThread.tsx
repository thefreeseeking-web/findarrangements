...
export default function ChatThread({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: otherProfile } = await supabase
        ...
        .eq('id', otherUserId)
        .maybeSingle();
      if (!cancelled) setOtherName(otherProfile?.display_name ?? 'Member');
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="px-4 sm:px-6 py-3 flex items-center gap-3 border-b border-white/10 flex-shrink-0">
        {onBack && (
          <button onClick={onBack} style={{ color: 'var(--muted)' }}>←</button>
        )}
        {otherId && (
          <Link href={`/profile/${otherId}`} className="font-display text-lg" style={{ color: 'var(--cream)' }}>
            {otherName}
          </Link>
        )}
      </div>

      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-2 min-h-0">
        {messages.length === 0 && (
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} group`}>
              ...
              >
                {m.content}
                <div className="text-[10px] mt-1 opacity-60" style={{ color: mine ? '#1a1014' : 'var(--muted)' }}>
        ...
        {otherTyping && (
          <div className="flex justify-start">
            <div className="px-4 py-2 rounded-2xl text-sm italic" style={{ backgroundColor: 'var(--surface)', color: 'var(--muted)' }}>
              {otherName} is typing...
            </div>
          </div>
        )}
      ...
      <form
        onSubmit={handleSend}
        className="px-4 sm:px-6 py-3 border-t border-white/10 flex gap-2 sm:gap-3 flex-shrink-0"
